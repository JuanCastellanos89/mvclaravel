...
@section('contenido')
  ...
  <table class="table table-striped">
    ...
    <tbody>
      ...
      <tr>
        ...
        <td>
          @can('editar-usuarios')
          <a class="btn btn-sm btn-success" href="{{ route('usuario.edit',$usuario->id) }}"><i class="fa fa-fw fa-edit"></i> Editar</a>
          @endcan
          @can('borrar-usuarios')
          <button type="button" class="btn btn-danger btn-sm bg-red-500" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-id="{{ $usuario->id }}"> Eliminar</button>
          @endcan
        </td>
      </tr>
      @empty
  ...
  <script>
    ...
    exampleModal.addEventListener('show.bs.modal', function(event) {
      ...
      modalTitle.textContent = `Eliminar el usuario con el ID ${recipient}`
      var deleteForm = exampleModal.querySelector('#deleteForm')
      deleteForm.setAttribute('action', `http://mvclaravel.test/dashboard/usuario/${recipient}`)
    })
  </script>
@endsection