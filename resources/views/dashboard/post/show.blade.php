  ...
  @foreach ($replys as $reply)
    ...
    <div class="row ml-2">
      ...
      <div class="col-1 mt-2">
        <a href="{{ route('reply.edit', $reply->id) }}" class="btn btn-sm btn-success" title="editar">Editar</a>          
        <button type="button" class="btn btn-danger btn-sm bg-red-500" data-bs-toggle="modal" data-bs-target="#exampleModal" data-bs-id="{{ $reply->id }}">Eliminar</button>
      </div>
    </div>
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>¿Seguro que desea eliminar la respuesta?</p>
          </div>
    ...
    <script>
      ...
      exampleModal.addEventListener('show.bs.modal', function(event) {
        ...
        modalTitle.textContent = `Eliminar respuesta con el id ${recipient}`
        var deleteForm = exampleModal.querySelector('#deleteForm')
        deleteForm.setAttribute('action', `http://mvclaravel.test/public/dashboard/reply/${recipient}`)
      })
    </script>
    @endif
  @endforeach  
